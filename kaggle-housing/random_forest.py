...
class RandomForest(IModel):
    ...
    def predict(self, x, x_test, cv=False):
        """
        IModel's predict functions random forest implementation.
        ...
        logging.info("Performing Random Forest prediction...")
        ...
        if cv:
            ...
            y = y.SalePrice.ravel()
            scores = cross_validate(model, x, y, cv=n_split, scoring=scoring, return_estimator=True)
            estimators = scores["estimator"]
            logs = [f'Model evaluation {i + 1} of {n_split}: {-1 * scores["test_score"][i]}' for i in range(n_split)]
            logging.info('\nRandom forest cross validation MAE:\n' + '\n'.join(logs))
            ...
            logging.info(f"Random forest validation MAE: {mean_absolute_error(y_valid, prediction)}")
        ...
        test_pred.to_csv(f'submissions\\submission_{"random_cv" if cv else "random"}.csv', index=False)
